...
<template>
  <div class="w-full flex flex-col gap-9">
    ...
    >
      <p
        class="poppins-normal font-bold text-[11px] md:text-xl text-gray-700"
      >{{ props.titulo }}</p>
      <i 
        :class=props.icono
    ...
    <div class="flex flex-col px-5 md:px-8 w-full">
      <p
      v-if="props.monto > 0 || props.titulo === 'Eventos Activos'"
        class="text-xl md:text-2xl font-bold mb-3 text-gray-700"
        :class="
    ...
    >
      ...
      <p
        class="truncate md:text-sm text-gray-900"
      >{{ props.texto }}</p>
    </div>
  </div>
